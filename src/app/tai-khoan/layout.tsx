...
export default async function AccountLayout({ children }: { children: React.ReactNode }) {
  ...
  return <div className="account-shell"><div className="account-layout"><aside className="account-sidebar"><div className="account-user-card"><div className="account-avatar">{avatarUrl ? <img src={avatarUrl} alt="Avatar" className="w-full h-full object-cover" /> : userName.charAt(0)}</div><div className="account-user-name">{userName}</div><div className="account-user-email">{userEmail}</div></div><nav className="account-nav">{links.map(([href, label, Icon]) => <Link key={href} href={href} className="account-nav-link"><Icon size={16} />{label}</Link>)}</nav></aside><main className="account-content">{children}</main></div></div>;
}