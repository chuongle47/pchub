...
export default function OrderDetailPage({ params }: Props) {
  ...
  const steps = [
    ...
    { label: 'Đang chuẩn bị hàng', active: order.status !== 'cancelled' },
    {
      ...
      active: order.status === 'shipping' || order.status === 'delivered',
    },
    ...
    <div className="bg-white border rounded-2xl p-6 shadow-sm">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mt-4 pb-4 border-b border-gray-100">
        <div>
          ...
          <p className="font-mono text-blue-600 mt-1 text-sm font-semibold">
            {order.id}
          </p>
        </div>
        <span
          className={`inline-block px-3 py-1.5 rounded-full text-xs font-bold mt-2 md:mt-0 ${
      ...
      <div className="grid md:grid-cols-2 gap-6 mt-6">
        ...
        <div className="bg-gray-50 dark:bg-gray-800/40 p-4 rounded-xl">
          <strong className="text-gray-900 dark:text-white text-sm block mb-3">
            Thông tin giao nhận
          </strong>
          ...
          <strong className="text-gray-900 dark:text-white text-sm block mt-5 mb-2">
            Thanh toán
          </strong>
