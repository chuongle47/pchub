...
export default async function SearchPage({ searchParams }: Props) {
  ...
  const result = query ? await getProducts({ search: query, limit: 20 }) : { products: [] };
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-[1280px] mx-auto px-4">
        ...
        {query ? <ProductGrid products={products} /> : <div className="bg-white border rounded-xl p-12 text-center text-gray-500">Hãy nhập từ khóa trong ô tìm kiếm phía trên.</div>}
      </div>
    </div>
  );
}
