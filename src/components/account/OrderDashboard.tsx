  ...
  return <>
    ...
    {featured && (activeFilter === 'all' || activeFilter === featured.status) && (
      <section className="order-featured">
        ...
        <div className="order-actions">
          <button type="button" onClick={() => setSelectedOrder(featured)}>Xem nhanh</button>
          <Link href={`/tai-khoan/don-hang/${featured.id}`} style={{ background: 'var(--color-primary)', color: '#fff', padding: '8px 16px', borderRadius: '8px', fontSize: '13px', fontWeight: 600, textDecoration: 'none', display: 'inline-flex', alignItems: 'center' }}>
            Chi tiết đơn hàng →
          </Link>
        </div>
      </section>
    ...
    <section className="recent-orders">
      <div className="recent-orders-heading">
        ...
        <Link href="/tai-khoan/don-hang" style={{ color: '#2563eb', fontSize: '13px', fontWeight: 600, textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
