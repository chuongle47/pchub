...
export default async function ComparePage({ searchParams }: Props) {
  ...
  const products = slugs.length
    ? (await getProductsBySlugs(slugs)).products
    : [];
  const mappedProducts: CompareProduct[] = products.map((product: any) => ({
    id: product.id,
    slug: product.slug,
    nameVi: product.name,
  ...
  const specKeys = [...new Set(mappedProducts.flatMap(product => Object.keys(product.specs)))];
  const columns = { display: 'grid', gridTemplateColumns: `150px repeat(${Math.max(mappedProducts.length, 1)}, minmax(190px, 1fr))` } as const;
